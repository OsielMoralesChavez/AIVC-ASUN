...
/** Campos que el docente aporta explícitamente al asistente (no se infieren del PDF). */
...
export interface PrimeraClaseContent extends UnirBaseContent {
  ...
  // Catálogo de 30 láminas de la skill "primera-clase-unir" — ver plan en
  // C:\Users\osiel\.claude\plans (sección "Actualizar primera clase y repaso...").
...
/** Tipos de deck que pueden servir de insumo para un repaso (todo salvo el propio repaso). */
export type RepasoSourceContent = PrimeraClaseContent | NormalLikeContent;

/** Ya no lleva sessionId: la presentación pertenece directamente al único usuario local
 * (biblioteca permanente, ver lib/db/presentationRepository.ts). */
export interface UnirPresentation {
  id: string;
